...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >= 0 && index < ll_len(this) && ll_len(this) > 0)
    {
        ...
        else
        {
            ...
            Node* pNodeAEliminar = pNode->pNextNode;
            pNode->pNextNode = pNodeAEliminar->pNextNode;
            free(pNodeAEliminar);
        }
        free(pNode);
        this->size--;
    ...
    if(this != NULL && pFunc != NULL && (order == 1 || order == 0))
    {
        ...
        for(int i=0; i<len-1; i++)
        {
            for(int j=i+1; j<len; j++)
            {
                ...
                if((pFunc(elementI, elementJ) > 0 && order == 1) || (pFunc(elementI, elementJ) < 0 && order == 0))
                {
 ...
 * \param Funcion que filtra el elemento. Devuelve (1) si el elemento es devuelto en la lista nuevo - (0) si no.
 * \return Retorna un nuevo LinkedList cargado de los elementos que cumplen con la funcion.
 *
 */
